import os
class config:
    # 取完整路径
    MemProcFsDir = os.path.abspath(r'..\Tools\MemProcFS\MemProcFS.exe')
    volatility2 = os.path.abspath(r'..\Tools\volatility2\vol.exe')
    volatility3 = os.path.abspath(r'..\Tools\volatility3\vol.py')
    volatility3_symbols = os.path.abspath(r'..\Tools\volatility3\symbols')
    gimp = os.path.abspath(r'..\Tools\gimp\bin\gimp-console-2.10.exe')
    pythonpath = os.path.abspath(r'..\python\python.exe')
    strings = os.path.abspath(r'..\Tools\other\strings.exe')

    profile  = ['Win7SP1x64', 'Win7SP1x86', 'WinXPSP3x86', 'WinXPSP2x86', 'WinXPSP2x64', 
                "VistaSP0x64","VistaSP0x86","VistaSP1x64","VistaSP1x86","VistaSP2x64",
                "VistaSP2x86","Win10x64","Win10x64_10586","Win10x64_14393","Win10x86",
                "Win10x86_10586","Win10x86_14393","Win2003SP0x86","Win2003SP1x64",
                "Win2003SP1x86","Win2003SP2x64","Win2003SP2x86","Win2008R2SP0x64",
                "Win2008R2SP1x64","Win2008R2SP1x64_23418","Win2008SP1x64","Win2008SP1x86",
                "Win2008SP2x64","Win2008SP2x86","Win2012R2x64","Win2012R2x64_18340","Win2012x64",
                "Win2016x64_14393","Win7SP0x64","Win7SP0x86","Win7SP1x64","Win7SP1x64_23418","Win7SP1x86",
                "Win7SP1x86_23418","Win81U1x64","Win81U1x86","Win8SP0x64","Win8SP0x86",
                "Win8SP1x64","Win8SP1x64_18340","Win8SP1x86","WinXPSP1x64","WinXPSP2x64","WinXPSP2x86","WinXPSP3x86"]


    

class llm_openai:
    openaikey = '' # openai api key
    baseurl = 'https://api.openai.com/v1/chat/completions' # openai api url
    model = 'gpt-4o-mini' # openai model
    proxy = 'http://127.0.0.1:1090'# 本地代理地址

