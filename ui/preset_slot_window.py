...
from PySide6.QtWidgets import (
    ...
    QPushButton, QGroupBox, QSizeGrip, QFrame
)
...
class PresetSlotWindow(QWidget):
    """预设槽独立窗口"""
    # 定义关闭信号，用于在窗口关闭时通知主窗口
    closed = Signal(object)
    
    def __init__(self, preset_slot):
        super().__init__(None, Qt.Window)  # 使用Qt.Window标志创建顶级窗口
        self.setWindowTitle("预设槽")
        ...
        self.preset_slot = preset_slot
        
        # 先设置父控件，确保控件可见
        self.preset_slot.setParent(self)
        
        self.setup_ui()
        
        # 确保预设槽可见
        self.preset_slot.setVisible(True)
    ...
    def setup_ui(self):
        ...
        # 添加预设槽到内容区域
        content_layout.addWidget(self.preset_slot)
    ...
    def closeEvent(self, event):
        """窗口关闭时发出信号"""
        self.closed.emit(self.preset_slot)
